...
st.caption("웨어러블 및 환경 데이터 기반 맞춤형 개입 시뮬레이션")
st.markdown("---")
...
with col1:
    caffeine = st.slider("☕️ 카페인 섭취량 (mg)", min_value=0, max_value=300, value=100, step=10)
    exercise = st.slider("🏃‍♂️ 취침 전 운동 강도 (0:없음 ~ 5:고강도)", min_value=0, max_value=5, value=2, step=1)

with col2:
    temp = st.slider("🌡️ 침실 온도 (섭씨)", min_value=15, max_value=28, value=22, step=1)
    noise = st.slider("🔊 침실 소음 레벨 (dB)", min_value=30, max_value=80, value=50, step=5)
    avg_hr = st.slider("❤️ 취침 전 평균 심박수 (BPM)", min_value=50, max_value=100, value=70, step=1)


if st.button("✨ 수면 패턴 예측 및 최적화 개입 받기", use_container_width=True):
    with st.spinner('AI가 수면 패턴을 예측 중...'):
        # 1. 수면 단계 예측 및 패턴 분석
